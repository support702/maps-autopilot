...
export const listProjectsTool = {
    ...
    handler: async (input, { ctx }) => {
        ...
        const groupedByOrg = projects.data.reduce((acc, project) => {
        ...
        const contents = Object.values(groupedByOrg)
            ...
            for (const project of org.projects) {
                parts.push(`- ${project.name} (projectRef=${project.externalRef}) (slug=${project.slug}) (createdAt=${project.createdAt})`);
            }
            return parts.join("\n");
        })
            .join("\n");
...
export const initializeProjectTool = {
    ...
    handler: toolHandler(InitializeProjectInput.shape, async (input, { ctx }) => {
        ...
        let projectRef = input.projectRef;
        if (!projectRef) {
            const cwd = input.cwd ?? (await ctx.getCwd());
            if (!cwd) {
                return respondWithError("No current working directory found. Please provide a projectRef or a cwd.");
            }
            ...
            if (config?.configFile) {
                if (typeof config.project === "string" && config.project.startsWith("proj_")) {
                    ...
                    return respondWithError("Could not find the project ref in the config file. Please provide a projectRef.");
                }
            }
            const cliApiClient = await ctx.getCliApiClient();
            const project = await cliApiClient.createProject(input.orgParam, {
            ...
            if (!project.success) {
                return respondWithError(`Failed to create project ${input.projectName} in organization ${input.orgParam}: ${project.error}`);
            }
        ...
        const manualSetupGuide = await getManualSetupGuide(projectRef, projectEnv.success ? projectEnv.data.apiKey : undefined, projectEnv.success ? projectEnv.data.apiUrl : undefined);
        return {
            content: [
                {
                    type: "text",
...
async function getManualSetupGuide(projectRef, apiKey, apiUrl) {
    ...
    text = text.replace("https://your-trigger-instance.com", apiUrl ?? "https://your-trigger-instance.com");
    return `
