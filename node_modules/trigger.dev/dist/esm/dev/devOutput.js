...
import { createTaskMetadataFailedErrorStack, TaskIndexingImportError, TaskMetadataParseError, } from "@trigger.dev/core/v3/errors";
import { TaskRunErrorCodes } from "@trigger.dev/core/v3/schemas";
import { aiHelpLink, chalkError, chalkGrey, chalkLink, chalkRun, chalkSuccess, chalkTask, chalkWarning, chalkWorker, cliLink, isLinksSupported, prettyError, prettyPrintDate, } from "../utilities/cliOutput.js";
import { eventBus } from "../utilities/eventBus.js";
import { logger } from "../utilities/logger.js";
import { BundleError } from "../build/bundle.js";
import { analyzeWorker } from "../utilities/analyze.js";
export function startDevOutput(options) {
    const { dashboardUrl, config } = options;
    const baseUrl = `${dashboardUrl}/projects/v3/${config.project}`;
    const rebuildStarted = (...[target]) => {
        logger.log(chalkGrey("○ Rebuilding local worker…"));
    };
    const buildStarted = (...[target]) => {
        logger.log(chalkGrey("○ Building local worker…"));
    };
    const buildFailed = (...[target, error]) => {
    ...
    const backgroundWorkerInitialized = (...[worker]) => {
    ...
    const backgroundWorkerIndexingError = (...[buildManifest, error]) => {
        if (error instanceof TaskIndexingImportError) {
            let errorText = "";
            for (const importError of error.importErrors) {
                prettyError(`Could not import ${importError.file}`, importError.stack ?? importError.message);
