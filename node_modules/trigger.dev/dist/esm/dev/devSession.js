import { bundleWorker, createBuildManifestFromBundle, getBundleResultFromBuild, logBuildFailure, logBuildWarnings, } from "../build/bundle.js";
...
export async function startDevSession({ rawConfig, name, rawArgs, client, dashboardUrl, keepTmpFiles, }) {
    ...
    async function updateBundle(bundle, workerDir) {
        ...
        await writeJSONFile(join(workerDir?.path ?? destination.path, "metafile.json"), bundle.metafile);
        ...
        try {
            ...
            await runtime.initializeWorker(buildManifest, bundle.metafile, workerDir?.remove ?? (() => { }));
        }
        catch (error) {
    ...
    async function updateBuild(build, workerDir) {
        const bundle = await getBundleResultFromBuild("dev", rawConfig.workingDir, rawConfig, build, storeDir);
        if (bundle) {
