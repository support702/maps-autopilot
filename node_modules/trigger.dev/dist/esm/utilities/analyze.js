...
export function analyzeWorker(worker, printDetails = false, disableWarnings = false) {
    if (!worker.manifest) {
        return;
    }
    if (printDetails) {
...
export function printBundleTree(workerManifest, metafile, opts) {
...
export function printBundleSummaryTable(workerManifest, metafile, opts) {
    const data = getBundleTreeData(workerManifest, metafile);
...
export function printWarnings(workerManifest) {
    ...
    for (const [filePath, timing] of Object.entries(timings)) {
        ...
        if (timing > 1000) {
            ...
            console.log("");
            console.log(chalkError("Warning: Slow import timing detected (>1s). This will cause slow startups. Consider optimizing this file."));
            console.log("");
        }
    }
    if (hasWarnings) {
...
function getBundleTreeData(workerManifest, metafile) {
    const tasksByFile = getTasksByFile(workerManifest.tasks);
    ...
    for (const filePath of Object.keys(tasksByFile)) {
        ...
        if (outputFile && outputs[outputFile]) {
            ...
            const seen = new Set();
            bundleChildren.forEach((child, idx) => {
...
function sumBundleTree(outputs, current, seen) {
    if (seen.has(current)) {
...
function formatSizeColored(bytes, withBraces = false) {
    let str;
...
function formatTimingColored(timing, withBraces = false) {
    let str;
...
function getDefinesTaskLabel(taskIds, prefix = "<-- defines tasks: ") {
    if (!taskIds) {
        return "";
    }
    if (taskIds.size === 0) {
...
function printBundleTreeNode({ outputs, current, branchSeen, taskSeen, prefix = "", isLast = true, preservePath = true, colorBundleSize = false, outputDefinesTaskIds = {}, }) {
    // Detect circular dependencies
    if (branchSeen.has(current)) {
        const displayPath = preservePath ? current : normalizePath(current);
        console.log(prefix + (isLast ? "└── " : "├── ") + chalk.grey(displayPath) + chalk.grey(" (circular)"));
        return;
    }
    ...
    const definesTaskLabel = output && !output.entryPoint ? getDefinesTaskLabel(outputDefinesTaskIds[current]) : "";
    console.log(prefix + (isLast ? "└── " : "├── ") + chalk.bold(displayPath) + ` ` + sizeStr + definesTaskLabel);
    // Print the children
...
function sumBundleTreeUnique(outputs, root) {
    const seen = new Set();
...
function countUniqueBundles(outputs, root) {
    const seen = new Set();
    function walk(current) {
        if (seen.has(current))
            return;
...
function printBundleRoot({ outputs, outputFile, preservePath, indent = "      ", taskSeen, outputDefinesTaskIds, }) {
    if (!outputFile) {
        return;
    }
    ...
    console.log(`${indent}└── ${chalk.bold(rootBundleDisplayPath)} ${rootBundleSizeColored}${definesTaskLabel}`);
...
function getInternalChildren(output, outputs) {
    return (output.imports || [])
        .filter((imp) => !imp.external && outputs[imp.path])
        .map((imp) => imp.path);
}
function findOutputFileByEntryPoint(outputs, entryPoint) {
    for (const [outputPath, outputMeta] of Object.entries(outputs)) {
...
function formatBundleLabel(count, size) {
    return `${count} bundle${count === 1 ? "" : "s"} ${size}`;
}
function getDisplayPath(path, preserve) {
    return preserve ? path : normalizePath(path);
}
function sortBundleTableData(a, b) {
    ...
    if (b.bundleSize !== a.bundleSize)
        return b.bundleSize - a.bundleSize;
    return a.filePath.localeCompare(b.filePath);
}
function printTaskTree(taskIds, indent = "", colorFn = chalkTask) {
    taskIds.forEach((id, idx) => {
        const isLast = idx === taskIds.length - 1;
        const symbol = isLast ? "└──" : "├──";
...
function printSlowImportTips() {
    ...
    console.log("- Are there a lot of tasks in this file? Consider splitting it into multiple files with a single task per file.");
    console.log("- Are you importing any tasks? Consider importing only the task types and trigger with `tasks.trigger(<task-id>)` instead. See: https://trigger.dev/docs/triggering#tasks-trigger");
    console.log("- Are there expensive operations outside of your task at the top level of the file? Consider moving them inside the task or only running them on demand by moving them into a function.");
    console.log("- Are you importing large libraries or modules that aren't used in all code paths? Consider importing them only when needed, for example with a dynamic `await import()`.");
    console.log("- Are you using third-party packages that are known to be slow to import? Check if there are lighter alternatives or if you can import only specific submodules.");
    console.log("");
