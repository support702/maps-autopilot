...
export function configureMcpCommand(program) {
    return commonOptions(program
        ...
        .option("--dev-only", "Only run the MCP server for the dev environment. Attempts to access other environments will fail.")
        ...
        .addOption(new CommandOption("--rules-install-branch <branch>", "The branch to install the rules from").hideHelp())).action(async (options) => {
...
export async function mcpCommand(options) {
    ...
    const fileLogger = options.logFile
