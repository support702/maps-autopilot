...
export function configurePreviewCommand(program) {
    ...
    commonOptions(preview
        ...
        .option("-b, --branch <branch>", "The preview branch to archive. If not provided, we'll detect your local git branch.")
...
export async function previewArchiveCommand(dir, options) {
    return await wrapCommandAction("previewArchiveCommand", PreviewCommandOptions, options, async (opts) => {
        return await _previewArchiveCommand(dir, opts);
    });
}
async function _previewArchiveCommand(dir, options) {
    ...
    if (!branch) {
        throw new Error("Didn't auto-detect branch, so you need to specify a preview branch. Use --branch <branch>.");
    }
...
export async function archivePreviewBranch(authorization, branch, project) {
