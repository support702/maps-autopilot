...
import { CommonCommandOptions, OutroCommandError, SkipCommandError, SkipLoggingError, commonOptions, handleTelemetry, tracer, wrapCommandAction, } from "../cli/common.js";
import { loadConfig } from "../config.js";
import { cliLink } from "../utilities/cliOutput.js";
import { createFileFromTemplate, generateTemplateUrl, } from "../utilities/createFileFromTemplate.js";
...
export function configureInitCommand(program) {
    return commonOptions(program
        ...
        .option("-p, --project-ref <project ref>", "The project ref to use when initializing the project")
        .option("--javascript", "Initialize the project with JavaScript instead of TypeScript", false)
        .option("-t, --tag <package tag>", "The version of the @trigger.dev/sdk package to install", cliVersion)
        .option("-r, --runtime <runtime>", "Which runtime to use for the project. Currently only supports node and bun", "node")
        ...
        .option("--pkg-args <args>", "Additional arguments to pass to the package manager, accepts CSV for multiple args")
        .option("-y, --yes", "Skip all prompts and use defaults (requires --project-ref)"))
        .addOption(new CommandOption("--git-ref <git ref>", "The git ref to use when fetching templates from GitHub").hideHelp())
...
async function _initCommand(dir, options) {
    ...
    const selectedProject = await selectProject(apiClient, authorization.dashboardUrl, options.projectRef);
    span?.setAttributes({
    ...
    if (!options.skipPackageInstall) {
        await installPackages(cwd, options.tag, new CLIInstallPackagesOutputter(options.logLevel, options.tag));
    }
    else {
    ...
    const projectDashboard = cliLink("project dashboard", `${authorization.dashboardUrl}/projects/v3/${selectedProject.externalRef}`);
    log.success("Successfully initialized your Trigger.dev project 🫡");
    log.info("Next steps:");
    log.info(`   1. To start developing, run ${chalk.green(`npx trigger.dev@${cliTag} dev${options.profile ? "" : ` --profile ${options.profile}`}`)} in your project directory`);
    log.info(`   2. Visit your ${projectDashboard} to view your newly created tasks.`);
    log.info(`   3. Head over to our ${cliLink("v3 docs", "https://trigger.dev/docs")} to learn more.`);
    log.info(`   4. Need help? Join our ${cliLink("Discord community", "https://trigger.dev/discord")} or email us at ${chalk.cyan("help@trigger.dev")}`);
    outro(`Project initialized successfully. Happy coding!`);
}
async function createTriggerDir(dir, options, language) {
    return await tracer.startActiveSpan("createTriggerDir", async (span) => {
        try {
            ...
            const templateUrl = generateTemplateUrl(`examples/${example}.${language === "typescript" ? "ts" : "mjs"}`, options.gitRef);
...
async function addConfigFileToTsConfig(tsconfigPath, options) {
...
class CLIInstallPackagesOutputter {
    logLevel;
    tag;
    installSpinner;
    ...
    stoppedWithError() {
        if (this.logLevel === "debug") {
            ...
            this.installSpinner.stop(`Failed to install @trigger.dev/sdk@${this.tag}. Rerun command with --log-level debug for more details.`);
        }
    }
}
class SilentInstallPackagesOutputter {
    startSDK() { }
    installedSDK() { }
    startBuild() { }
    installedBuild() { }
...
export async function installPackages(projectDir, tag, outputter = new SilentInstallPackagesOutputter()) {
    try {
        outputter.startSDK();
...
async function writeConfigFile(dir, project, options, triggerDir, language) {
    return await tracer.startActiveSpan("writeConfigFile", async (span) => {
        try {
            const spnnr = spinner();
            spnnr.start("Creating config file");
            const projectDir = resolve(process.cwd(), dir);
            const outputPath = join(projectDir, `trigger.config.${language === "typescript" ? "ts" : "mjs"}`);
            const templateUrl = generateTemplateUrl(`trigger.config.${language === "typescript" ? "ts" : "mjs"}`, options.gitRef);
            span.setAttributes({
                "cli.projectDir": projectDir,
...
async function selectProject(apiClient, dashboardUrl, projectRef) {
        ...
        try {
            if (projectRef) {
                ...
                if (!projectResponse.success) {
                    log.error(`--project-ref ${projectRef} is not a valid project ref. Request to fetch data resulted in: ${projectResponse.error}`);
            ...
            if (projectsResponse.data.length === 0) {
                const newProjectLink = cliLink("Create new project", `${dashboardUrl}/projects/new?version=v3`);
            ...
            const projectData = projectsResponse.data.find((project) => project.externalRef === selectedProject);
            if (!projectData) {
...
async function tryResolveTsConfig(cwd) {
    try {
