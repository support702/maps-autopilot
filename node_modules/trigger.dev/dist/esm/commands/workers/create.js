...
import { CommonCommandOptions, commonOptions, handleTelemetry, OutroCommandError, wrapCommandAction, } from "../../cli/common.js";
...
export function configureWorkersCreateCommand(program) {
...
async function workersCreateCommand(dir, options) {
    return await wrapCommandAction("workerCreateCommand", WorkersCreateCommandOptions, options, async (opts) => {
        return await _workersCreateCommand(dir, opts);
    });
}
async function _workersCreateCommand(dir, options) {
    ...
    outro(`Successfully created worker ${newWorker.data.workerGroup.name} with token ${newWorker.data.token.plaintext}`);
}
