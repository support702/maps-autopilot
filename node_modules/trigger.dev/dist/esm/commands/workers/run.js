...
export function configureWorkersRunCommand(program) {
...
async function workersRunCommand(dir, options) {
    return await wrapCommandAction("workerRunCommand", WorkersRunCommandOptions, options, async (opts) => {
        return await _workersRunCommand(dir, opts);
    });
}
async function _workersRunCommand(dir, options) {
    if (!options.token) {
        throw new Error("You must provide a worker token to run a worker locally. Either use the `--token` flag or set the `TRIGGER_WORKER_TOKEN` environment variable.");
    }
