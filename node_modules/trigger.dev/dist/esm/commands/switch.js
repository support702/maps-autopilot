...
import { CommonCommandOptions, handleTelemetry, OutroCommandError, wrapCommandAction, } from "../cli/common.js";
...
export function configureSwitchProfilesCommand(program) {
    return program
        .command("switch")
...
export async function switchProfilesCommand(profile, options) {
...
export async function switchProfiles(profile, options) {
    ...
    const profileNames = Object.keys(authConfig.profiles).sort((a, b) => {
        ...
        if (a === authConfig.currentProfile)
            return -1;
        if (b === authConfig.currentProfile)
            return 1;
