...
export async function updateTriggerPackages(dir, options, embedded, requireUpdate) {
  ...
  ${mismatches.map((dep) => `- ${dep.name}@${dep.version}`).join("\n")}\n`);
    ...
    try {
        installSpinner.message(`Installing new package versions${packageManager ? ` with ${packageManager.name}` : ""}`);
        await installDependencies({ cwd: projectPath, silent: true });
    }
    catch (error) {
        installSpinner.stop(`Failed to install new package versions${packageManager ? ` with ${packageManager.name}` : ""}`);
...
export async function tryResolveTriggerPackageVersion(name, basedir) {
    try {
...
export async function getPackageJson(absoluteProjectPath, options) {
