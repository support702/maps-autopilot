...
export function configurePromoteCommand(program) {
    return commonOptions(program
        ...
        .option("-b, --branch <branch>", "The preview branch to promote when passing --env preview. If not provided, we'll detect your git branch.")
        .option("--skip-update-check", "Skip checking for @trigger.dev package updates")
        .option("-p, --project-ref <project ref>", "The project ref. Required if there is no config file. This will override the project specified in the config file.")).action(async (version, options) => {
...
export async function promoteCommand(version, options) {
...
async function _promoteCommand(version, options) {
    if (!version) {
        throw new Error("You must provide a version to promote like so: `npx trigger.dev@latest promote 20250208.1`");
    }
